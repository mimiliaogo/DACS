...
# [TODO:mimi] Strong augmentation from ProDA
...
def my_photometric_augment_pool():
    # FixMatch paper
    augs = [(AutoContrast, None, None),
            (Brightness, 0.9, 0.05),
            (Color, 0.9, 0.05),
...
class RandAugmentMC(object):
    def __init__(self, n, m):
        ...
        self.n = n
        self.m = m
        # TODO: separate geometric and photometric augmentation
        self.geometric_pool = my_geometric_augment_pool()
        self.photometric_pool = my_photometric_augment_pool()
    ...
    def __call__(self, img, type='crc'):
        ...
        if type == 'cr' or type == 'crc':
            # TODO: choose one geometric augmentation and several photometric augmentation
            ops = random.choices(self.geometric_pool, k=1) + random.choices(self.photometric_pool, k=self.n)
            for op, max_v, bias in ops:
...
if __name__ == '__main__':
    ...
    img_path = '/mnt/shared/engine211/Dataset/cityscapes/leftImg8bit/train/aachen/aachen_000084_000019_leftImg8bit.png'
    img = m.imread(img_path)
    img = np.array(img, dtype=np.uint8)
    # strong aug
    img, params = randaug(Image.fromarray(img))
    img = np.array(img)
    # img = img[:, :, ::-1]  # RGB -> BGR
